...
exports.signin = (req,res,next)=>{
  ...
  .then(user=>{
   if(!user){
    return res.status(400).json({success:false,body:{status:400,title:'Authentication Error',data:[{path:'email',msg:'Incorrect email address'}]}})
   }
   return bcrypt.compare(password,user.password)
   .then(doMatch=>{
    if(!doMatch){
    return res.status(400).json({success:false,body:{status:400,title:'Authentication Error',data:[{path:'password',msg:'Incorrect email address'}]}})  
    }
